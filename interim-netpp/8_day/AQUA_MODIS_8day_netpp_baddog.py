...
def main():
    ...
    for year in range(started_year, ended_year + 1):
        # Create dynamic directories based on sensor
        CHL_DIR = os.path.join(CHL_DIR_T.format(args.sensor))
        PAR_DIR = os.path.join(PAR_DIR_T.format(args.sensor))
    ...
    while current_date <= end_date:
        ...
        nc_file.title = ', '.join([
            ...
            "Global",
            "9km",
            "2023-present (8 Day Composite)".format([args.sensor])
            ])
