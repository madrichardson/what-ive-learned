# Finding Unbiased Relative Difference (psi)
...
NCO_DIR = '/home/madison/miniforge3/bin/'
...
# Create  function to populate a NetCDF file with a 3D matrix of the psi values
...
def main():
    ...
    nc_filename = f'psi_8DAY_3D_matrix_{args.comparison_sensor}_2023to2024.nc'
    ...
    for start_dt, end_dt in dates_to_bin:
        ...
        nc_file.title = (
            f"Pixel by pixel Psi, VIIRS {args.sensor.upper()} "
            f"minus {args.comparison_sensor.upper()}"
        )
        nc_file.summary = (
            f"The unbiased relative difference (Psi) between primary "
            f"productivity (NetPP) calculated using VIIRS "
            f"{args.sensor.upper()} data and "
            f"{args.comparison_sensor.upper()} data. For each pixel, psi is "
            f"calculated as netPP(VIIRS {args.sensor.upper()}) - "
            f"netPP({args.comparison_sensor.upper()}) divided by the mean of "
            f"netpp({args.sensor.upper()}) and "
            f"netPP({args.comparison_sensor.upper()}) "
            ...
            f"either {args.comparison_sensor.upper()} "
            f"or {args.sensor.upper()}."
        )
        nc_file.sync()
