...
CWUTL_DIR = '"/home/madison/cwutils/bin"'
...
def generate_url_batch(date_obj):
    ...
    base_url = "https://coastwatch.noaa.gov/erddap/files/" "noaacwLEOACSPOSSTL3SKDaily/"
...
def calculate_Z_eu(chl_squeeze):
    ...
    """
    chl_eu = ma.where(
        chl_squeeze > 1.0, 40.2 * chl_squeeze**0.5070, 38.0 * chl_squeeze**0.4250
    )
...
def upload_to_erddap(local_file_path, erddap_data_dir, erddap_host):
    """
    Upload a NetCDF file to the ERDDAP server's monitored data directory
    using public key authentication.

    Args:
        local_file_path (str): Path to the NetCDF file generated locally on
        the remote server.
        erddap_data_dir (str): Path to the monitored directory on the target
        ERDDAP server.
        erddap_host (str): Hostname or IP address of the ERDDAP server.

    Raises:
        Exception: If the scp transfer fails.
    """

    file_name = os.path.basename(local_file_path)
    remote_file_path = os.path.join(erddap_data_dir, file_name)

    # SCP command for public key authentication
...
def main():
    ...
    while current_date <= end_date:
        ...
        nc_filename = f"productivity_viirs_{args.sensor}_daily_{formatted_date}.nc"
        ...
        try:
            ...
            download_nasa_data(current_date, "par", PAR_DIR, args.sensor)
        except Exception as e:
            print(
                f"Failed to download one or more of the NASA files for "
                f"{current_date}: {e}"
            )
            current_date += timedelta(days=1)
            continue
        ...
        remote_host = "192.168.31.27"
        remote_data_dir = (
            f"/u00/madi/netpp/data/{args.sensor}/netpp/daily_netpp/{current_date.year}"
        )
        ...
        try:
            upload_to_erddap(
                ...
                erddap_data_dir=remote_data_dir,  # Target directory on ERDDAP server
                erddap_host=remote_host,  # ERDDAP server hostname or IP
            )
            print(f"File successfully uploaded to ERDDAP: {nc_file_path}")
        except Exception as e:
            print(f"Error during upload for {nc_file_path}: {e}")
